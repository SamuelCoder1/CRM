    ...
    try {
      ...
      if (before === true) {
        ...
        if (Math.round(dias) < 5) {
          toast.warn(`Tu suscripción vence en 📅 ${Math.round(dias)} ${Math.round(dias) === 1 ? 'dia' : 'dias'} `);
        }
        ...
        api.defaults.headers.Authorization = `Bearer ${data.token}`;
        setIsAuth(true);
        toastError(`Tu suscripción ha vencido 📅 ${vencimento}.
¡Ponte en contacto con el soporte para más información! 🛠️ `);
