              ...
              <Grid xs={12} sm={6} md={2} item>
                <FormControl margin="dense" variant="outlined" fullWidth>
                  ...
                  <Field
                    ...
                    margin="dense"
                  >
                    <MenuItem value="MENSUAL">{i18n.t("compaies.table.monthly")}</MenuItem>
...
export function CompaniesManagerGrid(props) {
  ...
  const renderPlanValue = (row) => {
    return row.planId !== null ? row.plan.amount ? row.plan.amount.toLocaleString('es', { minimumFractionDigits: 2 }) : '00.00' : "-";
  };
...
export default function CompaniesManager() {
  ...
  const handleSubmit = async (data) => {
    ...
    try {
      ...
      toast.error(
        "No fue posible realizar la operación ❌. Verifique se já existe uma empresa com o mesmo nome ou se os campos foram preenchidos corretamente"
      );
    }
    setLoading(false);
  };
