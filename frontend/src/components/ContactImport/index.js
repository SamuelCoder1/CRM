...
const ContactImport = () => {
  ...
  const processImport = async () => {
    ...
    if (!selection.number) {
      toastError("❌ No se seleccionó el campo del número de contacto 📞");
      setUploading(false);
      return;
    }

    if (!selection.name) {
      toastError("❌ No se seleccionó el campo del nombre del contacto 👤 ");
      setUploading(false);
      return;
    }

    if (Object.keys(selectedRows).length === 0) {
      toastError("❌ Ningún contacto seleccionado 👥");
      setUploading(false);
      return;
    }
    ...
    if (rows?.length > 1) {
      ...
      if (countIgnored === 0) {
        toast.success("✅ Importación realizada con éxito");
      } else {
        toast.alert("✅ Importación realizada con éxito, pero hubo algunos errores ⚠️");
      }
    }
  };
  ...
  const handleSelectChange = (event) => {
    ...
    if (selectedFields[newValue]) {
      toastError(`O campo ${newValue}  Ya ha sido seleccionado.`);
      return;
    }
    ...
    return (
      <div>
        ...
        <div className={classes.actions}>
          {uploading && <div>Importando... Espere</div>}
          <Button
            variant="contained"
            color="primary"
            disabled={uploading}
  ...
  return (
    <div style={{ alignContent: "center" }}>
      ...
      {openingFile && <div>⏳ Procesando archivo...</div>}
      ...
      {!imported && rows && columns ? renderContent() : (
        <>
          ...
          >
            ...
            <h5>Haz clic 📂 o arrastra 🖱️ un archivo 📁</h5>
            <p style={{ color: "#e74c3c", fontWeight: "bold", textAlign: "center" }}>
            ⚠️ Importante: Solo se aceptan archivos 📄 con las siguientes extensiones: xls, xlsx, csv, txt."
            </p>
          </div>
