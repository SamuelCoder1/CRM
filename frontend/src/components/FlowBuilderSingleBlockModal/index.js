...
const FlowBuilderSingleBlockModal = ({
  ...
  const handleElements = newNameFiles => {
    ...
    for (let i = 0; i < numberMessages; i++) {
      ...
      if (!value) {
        toast.error("¡Campo de mensaje vacío! ⚠️");
        setLoading(false);
        throw "";
      }
    ...
    for (let i = 0; i < numberInterval; i++) {
      ...
      if (parseInt(value) === 0 || parseInt(value) > 120) {
        toast.error("¡El intervalo no puede ser 0 o mayor que 120! ⏳⚠️");
        setLoading(false);
        throw "";
      }
      elementsSequence.push({
  ...
  const handleChangeMediasImg = (e, number) => {
    ...
    if (e.target.files[0].size > 2000000) {
      toast.error("El archivo es muy grande! Máximo 2MB 📂⚠️");
      return;
    }
  ...
  const handleChangeAudios = (e, number) => {
    ...
    if (e.target.files[0].size > 5000000) {
      toast.error("¡El archivo es muy grande! Máximo 5MB 📂⚠️");
      return;
    }
  ...
  const handleChangeVideos = (e, number) => {
    ...
    if (e.target.files[0].size > 20000000) {
      toast.error("¡El archivo es muy grande! Máximo 20MB 📂⚠️");
      return;
    }
      ...
      setTimeout(async () => {
        if (
          ...
          try {
            ...
            toast.success("¡Contenido añadido con éxito! 🎉✅");
            handleClose();
            setLoading(false);
        ...
        if (verify) {
          setLoading(false);
          return toast.error("Elimina las tarjetas vacías 🗑️(Imagem, Audio e Video)");
        }
        await api
