...
const FlowBuilderIntervalModal = ({
  ...
  const handleSaveContact = async values => {
    if(!timerSec || parseInt(timerSec) <= 0){
      return toast.error('Agrega un valor de intervalo ⏳');
    }
    if(parseInt(timerSec) > 120){
      return toast.error('Tiempo máximo alcanzado: 120 segundos ⏲️');
    }
    if(open === 'edit'){
      onUpdate({
        ...data,
    ...
    <div className={classes.root}>
      <Dialog open={activeModal} onClose={handleClose} fullWidth="md" scroll="paper">
        <DialogTitle id="form-dialog-title">
          {open === 'create' ? `Agregar un intervalo al flujo 🔄` : `Editar intervalo ✏️`}
        </DialogTitle>
        <Stack>
