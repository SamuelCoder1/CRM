            ...
            {(values) => (
                <Form className={classes.fullWidth}>
                    <Grid spacing={1} justifyContent="flex-start" container>
                        ...
                        <Grid xs={12} sm={8} md={2} item>
                            <FormControl margin="dense" variant="outlined" fullWidth>
                                <InputLabel htmlFor="useOpenAi-selection">prompt</InputLabel>
                                <Field
                                    as={Select}
                ...
                <TableBody>
                    {records.map((row) => (
                        <TableRow key={row.id}>
                            ...
                            <TableCell align="center">{i18n.t("plans.form.money")} {row.amount ? row.amount.toLocaleString('es', { minimumFractionDigits: 2 }) : '00.00'}</TableCell>
...
export default function PlansManager() {
    ...
    const loadPlans = async () => {
        ...
        try {
            ...
            toast.error('No fue posible cargar la lista de registros ⚠️')
        }
        setLoading(false)
    }
    ...
    const handleSubmit = async (data) => {
        ...
        try {
            ...
            toast.error('No fue posible realizar la operación. Verifica si ya existe un plan con el mismo nombre o si los campos fueron completados correctamente ⚠️')
        }
        setLoading(false)
    }
    ...
    const handleSelect = (data) => {
        ...
        setRecord({
            ...
            amount: data.amount?.toLocaleString('es', { minimumFractionDigits: 2 }) || 0,
            useWhatsapp,
            useFacebook,
            useInstagram,
            useCampaigns,
    ...
    return (
        <Paper className={classes.mainPaper} elevation={0}>
            ...
            <ConfirmationModal
                title="Eliminación de Registro 🗑️"
                open={showConfirmDialog}
                onClose={() => setShowConfirmDialog(false)}
                onConfirm={() => handleDelete()}
            >
                ¿Realmente deseas eliminar este registro? ❗🗑️
            </ConfirmationModal>
        </Paper>
    )
}